...
const { CONVERSATION_EVENTS, MESSAGE_EVENTS } = require('../../shared/constants');
...
class ConversationEventsHandler {
  ...
  async handleJoinConversation(conversationId, userId, socket) {
    try {
      ...
      socket.to(`conversation:${conversationId}`).emit(CONVERSATION_EVENTS.USER_JOINED, {
        userId,
        conversationId,
        joinedAt: new Date()
      });
  ...
  handleLeaveConversation(conversationId, socket) {
    ...
    socket.to(`conversation:${conversationId}`).emit(CONVERSATION_EVENTS.USER_LEFT, {
  ...
  async broadcastConversationUpdate(conversationId, updateType, data) {
    try {
      ...
      console.log(`📢 Broadcast ${updateType} for conversation ${conversationId}`);
  ...
  broadcastNewMessage(conversationId, messageData) {
    this.io.to(`conversation:${conversationId}`).emit(MESSAGE_EVENTS.NEW_MESSAGE, {
      conversationId,
      message: messageData,
      timestamp: new Date()
    });
  ...
  broadcastMessageRead(conversationId, messageId, userId) {
    this.io.to(`conversation:${conversationId}`).emit(MESSAGE_EVENTS.MESSAGE_READ_UPDATE, {
      conversationId,
      messageId,
      userId,
      readAt: new Date()
  ...
  broadcastMessageDelivered(conversationId, messageId, userId) {
    this.io.to(`conversation:${conversationId}`).emit(MESSAGE_EVENTS.MESSAGE_DELIVERY_UPDATE, {
      conversationId,
      messageId,
      userId,
